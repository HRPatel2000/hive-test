...
public class DealersHierarchyParquetTest {
    ...
    File setupHiveUDFFile = new File(HiveProperties.getInstance().getProperty("hive.udf.file")); //Hive UDFs
//    File setupHiveJarFile = new File(HiveProperties.getInstance().getProperty("hive.dependency.jar.file")); //Hive Jar Files
//    File setupHivePropertiesFile = new File(HiveProperties.getInstance().getProperty("hive.properties.file")); //Hive Properties
//    File repairHivePartitionsFile = new File(HiveProperties.getInstance().getProperty("hive.table.repair.file")); //Hive Partitions and DDL Changes Repair
    ...
    File setupCurrentTableFile = new File(HiveProperties.getInstance().getProperty("hive.current.parquet.table.file"));
    ...
    public void initialize() {
        ...
        hiveShell.addSetupScripts(
                ...
                setupHiveUDFFile, //Hive UDFs
//                setupHiveJarFile, //Hive Jar Files
//                setupHivePropertiesFile, //Hive Properties
//                repairHivePartitionsFile, //Hive Partitions and DDL Changes Repair
                setupTempDataTableFile,
                setupCoreTableFile,
                setupCurrentTableFile
        );
        ...
        DDLRepairUtil.repairTables(
                ...
                "dealer_hierarchy_current",
                "dealer_hierarchy_temp_data");

        String insertDataIntoCoreTable = null;
        String insertDataIntoCurrentTable = null;

        try {

            insertDataIntoCoreTable = HiveFileUtil.readFile(HiveProperties.getInstance().getProperty("hive.core.avro.table.data.file"));
            insertDataIntoCurrentTable = HiveFileUtil.readFile(HiveProperties.getInstance().getProperty("hive.current.parquet.table.data.file"));
        } catch (IOException e) {
            e.printStackTrace();
        }

        hiveShell.execute(insertDataIntoCoreTable);
        hiveShell.execute(insertDataIntoCurrentTable);
    }
    ...
    public void testTablesCreated() {
        List<String> expected = Arrays.asList("dealer_hierarchy_core", "dealer_hierarchy_current", "dealer_hierarchy_temp_data");
    ...
    @Test
    public void testDataCountFromCurrentTableParquetFormat() {
        ...
        actual = hiveShell.executeQuery("select COUNT(*) from dealer_hierarchy_current");
        Assert.assertEquals(expected, actual);
    }

    @Test
    public void testPartitionDataCountFromCurrentTableParquetFormat() {
        ...
        actual = hiveShell.executeQuery("select COUNT(*) from dealer_hierarchy_current where dealer_code='AAAA'");
        Assert.assertEquals(Arrays.asList("1"), actual);

        actual = hiveShell.executeQuery("select COUNT(*) from dealer_hierarchy_current where dealer_code='ZZZZ'");
        Assert.assertEquals(Arrays.asList("3"), actual);
    }

    @Test
    public void testCurrentTableContentByHash() {

        String expectedHash = null;
        String actualHash = null;

        //Adjust Input to account for UDFs Date/Timestamp Formatting
        String formattedInputForPartitionAAAA = "dealer_021205|021205|DMS|2.0|2008-12-18 12:12:12.000|2008-12-18 12:12:12  +00:00 +00:00|A001|C|A|01|AA|GENIUS_SPECTRUM|01|L100|L100|CAT|001|2010-10-30T12:12:12|2010-10-30T12:12:12|HPATEL|2010-10-30T12:12:12|2010-10-30T12:12:12|001|AAAA";
        ...
        List<String> actual = hiveShell.executeQuery("select * from dealer_hierarchy_current where dealer_code='AAAA'");
        if(!actual.isEmpty()) {
    ...
    @Test
    public void testCurrentTablePartitionCreated() {
        List<String> expected = Arrays.asList("dealer_code=AAAA", "dealer_code=ZZZZ");
        List<String> actual = hiveShell.executeQuery("SHOW PARTITIONS dealer_hierarchy_core");
        Assert.assertEquals(expected, actual);

        actual = hiveShell.executeQuery("SHOW PARTITIONS dealer_hierarchy_current");
        Assert.assertEquals(expected, actual);
    }
}
